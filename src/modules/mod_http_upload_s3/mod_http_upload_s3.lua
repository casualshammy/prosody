...
local aws_region = assert(module:get_option_string(module.name .. "_region"), module.name .. "_region is a required option");
local aws_bucket = assert(module:get_option_string(module.name .. "_bucket"), module.name .. "_bucket is a required option");
local aws_path = assert(module:get_option_string(module.name .. "_path"), module.name .. "_path is a required option");
local aws_access_id = assert(module:get_option_string(module.name .. "_access_id"), module.name .. "_aws_access_id is a required option");
local aws_secret_key = assert(module:get_option_string(module.name .. "_secret_key"), module.name .. "_aws_secret_key is a required option");
local host = module:get_option_string(module.name .. "_host", string.format("%s-%s.amazonaws.com", aws_service, aws_region));
...
local function get_derived_signing_key(_secretKey, timestamp, region, service)
	local date = get_iso8601_basic_short(timestamp)
	local k_secret = "AWS4" .. _secretKey
...
local function get_canonical_query_string(timestamp, credential, request_method, _filename)
	...
	for param, value in sorted_iter(query_params) do
		local encoded = encode_uri_component(param) .. "=" .. encode_uri_component(value)
...
local function get_public_get(_, _, uri)
...
local function handle_request(origin, stanza, xmlns, filename, filesize, filetype)
	...
	-- validate
	if not filename or filename:find("/") or filename:find("\\") then
